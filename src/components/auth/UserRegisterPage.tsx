...
export const UserRegisterPage: React.FC<UserRegisterPageProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (captured) {
      setReferralId(captured);
    }
  }, []);

  // Check sponsor name dynamically
  const referrerUser = allUsers.find(
    (u) =>
      u.referralCode?.toUpperCase() === referralId.trim().toUpperCase() ||
      u.id?.toUpperCase() === referralId.trim().toUpperCase()
  );
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = registerUser(
        ...
        referralId.trim().toUpperCase() || 'AT10001',
        stateOfResidence
      );

      if (res.success && res.user) {
        setGeneratedUserId(res.user.id);
        setSuccessMessage(`Account created successfully! Your unique User ID is ${res.user.id}. Redirecting to your Dashboard...`);
        setTimeout(() => {
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold mb-3 shadow-sm">
            ...
            <span>INSTANT ₹200 WELCOME BONUS ON SIGNUP</span>
          </div>
          ...
          {successMessage && (
            <div className="mb-5 p-4 rounded-xl bg-emerald-500/15 border border-emerald-500/40 text-emerald-300 text-xs font-bold flex items-center gap-2.5">
              ...
              <div>
                <p>{successMessage}</p>
                {generatedUserId && (
                  <p className="text-amber-300 font-mono text-sm mt-0.5">User ID: {generatedUserId}</p>
                )}
              </div>
            </div>
          )}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">
                  Referral ID / Referral Code
                </label>
                {referralId && referrerUser && (
                  <span className="text-[11px] font-bold text-emerald-400 flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" /> Sponsor: {referrerUser.name} ({referrerUser.id})
                  </span>
                )}
              </div>
              <div className="relative">
